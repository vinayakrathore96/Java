...
-->
		...
		<div>
                    <center><h4>Check Orders</h4></center>
		
<%@page import="java.sql.*;" %>
<jsp:useBean id="cn" scope="page" class="mm.Conn1" />
<center><table cellspacing="20"><tr><th>User Id</th><th>Product Id</th><th>Product Name</th><th>Quantity</th><th>Total Price</th><th>Status</th><th>Action</th></tr>
<%
     ResultSet rd=cn.executeQuery("select * from orders");
     while(rd.next()){
         %>
    <tr><td><%=rd.getString("user_id")%></td><td><%=rd.getString("p_id")%></td><td><%=rd.getString("p_name")%></td><td><%=rd.getString("qty")%></td><td><%=rd.getString("price")%></td><td><%=rd.getString("status")%></td>
        <td><a href="order_status.jsp?uid=<%=rd.getString("user_id")%>&pid=<%=rd.getString("p_id")%>&pname=<%=rd.getString("p_name")%>&uemail=<%=rd.getString("user_email")%>">Update Status</td></tr>
    <%}%>
    </table></center>
		</div>
	</div>
