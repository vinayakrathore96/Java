...
function check()
{
   ...
   if(f1.mobile.value=="" || f1.mobile.value==null){
    ...
    <body><form action="edit_profile1.jsp" method="post" name="f1">
        <table cellpadding="20"> <tr><th>Product Image</th><th>Product name</th><th>Price</th><th>Product_details</th><th>Action</th></tr>
            
        <% 
         
        ResultSet rs=cn.executeQuery("select * from products");
        while(rs.next()){
            %>
            <tr><td><img  src=products/<%=rs.getString("image")%> width=50px  height=50></td><td><%=rs.getString("product_name")%></td><td><%=rs.getString("price")%></td><td><%=rs.getString("Product_details")%></td><td><a href="delete_product.jsp?id=<%=rs.getString("product_id")%>">Delete</a></td></tr>
